...
#for payload, right click and select "inspect" and then click on Network and refresh to get process.click on process and then payload
...
###Intraday Scanners
virat100bullish = "( {cash} ( latest close > latest ema( latest close , 5 ) and latest rsi( 14 ) > 55 and latest close > latest open * 1.01 and latest volume > 200000 and latest close < 1500 and latest close > 1 day ago close and latest true range( 14 ) > latest avg true range( 14 ) and latest true range( 14 ) > 8 and latest close > 1 week ago high and latest close > ( latest open + 1 day ago high + 1 day ago close / 3 ) and market cap > 5000 and latest macd histogram( 26,12,9 ) > 0 and latest macd line( 26,12,9 ) > latest macd signal( 26,12,9 ) and latest close > 1 day ago high and latest close > 500 ) )"
BFSIntradayvwap = "( {33489} ( latest close > ( 1 day ago close + ( 1 day ago close * 0.01 ) ) and latest volume > ( 1 day ago volume + ( 1 day ago volume * 0.05 ) ) and [0] 5 minute close > [0] 5 minute vwap ) ) "
    ...
    while (isCorrectTimeToalert()):
        ...
        for itr in conditionArray:
            ...
            if (len(data)==0):
                ...
                SendMessageToTelegram(header + "No data")
                continue
            else:
